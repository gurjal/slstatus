...
void format_title(char *title, const char *mpd_title, size_t n) {
...
const char *media_mpd_stat() {
  struct mpd_connection *conn;
  struct mpd_status *status;
  struct mpd_song *song;
  char *mpc_title, title[32];
  char ret[64];
  unsigned state;
  int elapsed, total;
  ...
  if (status) {
    ...
    if (state == MPD_STATE_PLAY) {
      ...
      snprintf(ret, 64, "\uf01d %s (%.2d:%.2d/%.2d:%.2d)", title, elapsed / 60,
      ...
      snprintf(ret, 64, " ");
      // og code for displaying stop symbol
      //        mpd_response_next(conn);
